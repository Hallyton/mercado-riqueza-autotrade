...
export async function POST(request: Request) {
  ...
  const parsed = bulkExecuteSchema.safeParse(body ?? {});
  if (!parsed.success) {
  ...
  try {
    ...
    console.error("[admin/real-trading/bulk-dispatch/execute]", error);
    return jsonError("Falha ao executar disparo em lote", 500);
  }
}
