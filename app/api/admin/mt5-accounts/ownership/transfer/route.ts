...
export async function POST(request: Request) {
  ...
  const parsed = mt5OwnershipTransferSchema.safeParse(json ?? {});
  if (!parsed.success) {
    return NextResponse.json(
  ...
  try {
    ...
    console.error("[admin/mt5-accounts/ownership/transfer]", error);
    return NextResponse.json(
      { ok: false, message: "Falha ao transferir conta MT5." },
      { status: 500 }
    );
  }
}
