    ...
    <div className="space-y-8">
      ...
      {preview && (
        <section className="rounded-lg border border-red-500/30 bg-red-950/20 p-5 space-y-4">
          <h2 className="text-lg font-semibold text-red-200">Bloco 4 — Disparo controlado</h2>
          <p className="text-sm">
          ...
          <button
            type="button"
            disabled={busy || previewExpired || selectedCount === 0}
            onClick={onExecute}
          ...
          <ul className="space-y-1 text-xs font-mono">
            {executeResult.items.map((item) => (
              <li key={item.licenseId}>
                ...
                {item.instructionId ? ` (${item.instructionId})` : ""}
